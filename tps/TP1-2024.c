...
item posicionK(Lista*, int); // Funci�n externa - Punto 3) - FALTA IMPLEMENTAR 

// Se necesita pasar un puntero en algunas funciones

item posicionK (Lista *l, int k) {
    if (longitud(*l) < k || esListaVacia(*l)) return -99999;
    for (int i = 0; i < k - 1; i++)
    {
        borrar(l);
    }  
